...
function getRectangleString(width, height) {
  ...
  middle.unshift('│');
  middle.push('│\n');
  const bottom = new Array(width - 2).fill('─');
  bottom.unshift('└');
  bottom.push('┘\n');
  const result = height > 2 ? top.join('') + middle.join('').repeat(height - 2) + bottom.join('')
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i <= str.length - 1; i += 1) {
    let newLetter = str[i].charCodeAt() + 13;
    if (str[i].charCodeAt() > 64 && str[i].charCodeAt() < 91) {
      if (newLetter > 90) {
        newLetter -= 26;
      }
      message.push(String.fromCharCode(newLetter));
